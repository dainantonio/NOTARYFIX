...
const Landing = () => {
  ...
  return (
    <div className="min-h-screen bg-[#08152f] text-white">
      <div className="border-b border-white/10 bg-[#050e20]">
        <nav className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <div className="hidden items-center gap-6 text-sm text-slate-200 md:flex">
            {topLinks.map((item) => (
              <a key={item} href={item === 'Pricing' ? '#pricing' : item === 'FAQ' ? '#faq' : '#'} className="transition hover:text-white">
                {item}
              </a>
            ))}
          </div>
      ...
      <div className="border-b border-cyan-400/20 bg-cyan-500/10 px-6 py-2 text-center text-xs font-semibold tracking-wide text-cyan-100">
        {livePreview ? 'Interactive landing preview is ON — all cards, toggles, sliders, and FAQ are live.' : 'Interactive preview is paused.'}
        <button onClick={() => setLivePreview((v) => !v)} className="ml-3 rounded border border-cyan-300/40 px-2 py-0.5 text-[11px] font-bold text-cyan-100">{livePreview ? 'Pause Preview' : 'Resume Preview'}</button>
      </div>
        ...
        <div className="mx-auto grid max-w-6xl gap-8 px-6 lg:grid-cols-2">
          <div>
            ...
            <div className="mt-7 flex items-center gap-2 rounded-lg border border-slate-500 bg-[#11254a] p-2">
              <input className="w-full rounded-md bg-[#0b1d3b] px-3 py-2 text-sm text-slate-200 outline-none disabled:cursor-not-allowed disabled:opacity-60" value={aiInput} onChange={(e) => setAiInput(e.target.value)} disabled={!livePreview} placeholder="Ask anything about notary compliance..." />
              <button onClick={submitAi} disabled={!livePreview} className="rounded-md bg-gradient-to-r from-teal-400 to-cyan-500 px-4 py-2 text-xs font-bold text-[#03152d] disabled:cursor-not-allowed disabled:opacity-60">Ask AI</button>
            </div>
          </div>
      ...
      <section className="mx-auto grid max-w-6xl gap-8 px-6 py-16 lg:grid-cols-2">
        <div>
          ...
          <div className="mt-4 inline-flex rounded-lg border border-slate-500 p-1 text-sm">
            <button onClick={() => livePreview && setBeforeAfterMode('old')} className={`rounded-md px-4 py-1 ${beforeAfterMode === 'old' ? 'bg-rose-500/20 text-rose-300' : 'text-slate-300'} ${!livePreview ? 'cursor-not-allowed opacity-60' : ''}`}>The Old Way</button>
            <button onClick={() => livePreview && setBeforeAfterMode('new')} className={`rounded-md px-4 py-1 ${beforeAfterMode === 'new' ? 'bg-emerald-500/20 text-emerald-300' : 'text-slate-300'} ${!livePreview ? 'cursor-not-allowed opacity-60' : ''}`}>With NotaryOS</button>
          </div>
        ...
        <div className="rounded-2xl border border-slate-400/70 bg-[#0b1a37] p-6">
          ...
          <div className="mt-8 space-y-6">
            <div>
              ...
              <input type="range" min="1" max="40" value={weeklyAppointments} onChange={(e) => setWeeklyAppointments(Number(e.target.value))} disabled={!livePreview} className="w-full accent-cyan-400 disabled:opacity-50" />
            </div>
            <div>
              ...
              <input type="range" min="5" max="60" value={adminMinsPerAppt} onChange={(e) => setAdminMinsPerAppt(Number(e.target.value))} disabled={!livePreview} className="w-full accent-cyan-400 disabled:opacity-50" />
            </div>
          </div>
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-12 px-6 lg:grid-cols-2">
          <div className="relative flex min-h-[320px] items-center justify-center">
            <div className="absolute left-10 top-3 h-72 w-44 -rotate-12 rounded-[30px] border-2 border-slate-700 bg-slate-100 p-4 text-slate-900 shadow-2xl">
              <div className="mb-3 flex items-center justify-between"><p className="text-sm font-bold">Today&apos;s Route</p><button onClick={() => livePreview && setPhoneTab('route')} className={`rounded px-2 py-0.5 text-[10px] ${phoneTab === 'route' ? 'bg-blue-100' : 'bg-slate-200'} ${!livePreview ? 'cursor-not-allowed opacity-60' : ''}`}>View</button></div>
              {phoneTab === 'route' ? (
            ...
            <div className="absolute right-8 top-0 h-72 w-44 rotate-12 rounded-[30px] border-2 border-slate-700 bg-slate-100 p-4 text-slate-900 shadow-2xl">
              <div className="mb-3 flex items-center justify-between"><p className="text-sm font-bold">Payments</p><button onClick={() => livePreview && setPhoneTab('payments')} className={`rounded px-2 py-0.5 text-[10px] ${phoneTab === 'payments' ? 'bg-blue-100' : 'bg-slate-200'} ${!livePreview ? 'cursor-not-allowed opacity-60' : ''}`}>View</button></div>
              {phoneTab === 'payments' ? (
                <>
      ...
      <section id="pricing" className="border-y border-white/10 bg-[#1f2b42] py-16">
        <div className="mx-auto max-w-6xl px-6">
          ...
          <div className="mx-auto mt-6 inline-flex w-full justify-center rounded-xl border border-slate-500 p-1 text-sm">
            <button onClick={() => livePreview && setBillingPeriod('monthly')} className={`rounded-lg px-5 py-2 ${billingPeriod === 'monthly' ? 'bg-slate-600' : ''} ${!livePreview ? 'cursor-not-allowed opacity-60' : ''}`}>Monthly</button>
            <button onClick={() => livePreview && setBillingPeriod('yearly')} className={`rounded-lg px-5 py-2 ${billingPeriod === 'yearly' ? 'bg-slate-600' : ''} ${!livePreview ? 'cursor-not-allowed opacity-60' : ''}`}>Yearly</button>
          </div>
          ...
          {faqItems.map((item, idx) => {
            ...
            return (
              <div key={item.question} className="rounded-xl border border-slate-600 bg-[#0d1d3c]">
                <button className="flex w-full items-center justify-between px-5 py-4 text-left" onClick={() => livePreview && setOpenFaq(isOpen ? -1 : idx)} aria-expanded={isOpen}>
