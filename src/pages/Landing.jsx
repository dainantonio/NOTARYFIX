...
const Landing = () => {
  ...
  const stackMonthlyCost = Math.round(comparisonSignings * 4.5 + 89);
  const notaryOsMonthlyCost = billingPeriod === 'monthly' ? 19 : 15;
  const monthlySavings = Math.max(0, stackMonthlyCost - notaryOsMonthlyCost);
    ...
    <div className="min-h-screen bg-[#08152f] text-white">
          ...
          <div className="hidden items-center gap-6 text-sm text-slate-200 md:flex">
            {topLinks.map((item) => (
              item === 'Pricing' ? (
                ...
                <a key={item} href={item === 'Features' ? '#features' : item === 'How it works' ? '#how-it-works' : '#faq'} className="transition hover:text-white">
                  {item}
                </a>
              )
            ))}
      ...
      <section id="features" className="mx-auto grid max-w-6xl gap-8 px-6 py-16 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-400/70 bg-[#0b1a37] p-6">
          ...
          <div className="mt-8 space-y-6">
            ...
            <div>
              <div className="mb-2 flex items-center justify-between text-sm text-slate-300"><span>YOUR HOURLY VALUE</span><span className="text-cyan-300">${hourlyRate}/hr</span></div>
              <input type="range" min="25" max="150" step="5" value={hourlyRate} onChange={(e) => setHourlyRate(Number(e.target.value))} className="w-full accent-cyan-400" />
            </div>
          </div>
      ...
      <section id="pricing" className="border-y border-white/10 bg-[#1f2b42] py-16">
        <div className="mx-auto max-w-6xl px-6">
          ...
          <div className="mt-10 rounded-xl border border-cyan-400/30 bg-[#0d1f41] p-5">
            <p className="text-xs font-semibold tracking-wide text-cyan-300">INTERACTIVE COST COMPARISON</p>
            <div className="mt-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <p className="text-sm text-slate-200">Monthly signings</p>
              <div className="w-full md:w-2/3">
                <input type="range" min="5" max="120" value={comparisonSignings} onChange={(e) => setComparisonSignings(Number(e.target.value))} className="w-full accent-cyan-400" />
              </div>
              <p className="text-lg font-bold text-cyan-300">{comparisonSignings}</p>
            </div>
            <div className="mt-4 grid gap-3 text-sm md:grid-cols-3">
              <div className="rounded-lg border border-slate-600 bg-[#10284f] p-3"><p className="text-slate-400">Typical Tool Stack</p><p className="text-2xl font-bold">${stackMonthlyCost}/mo</p></div>
              <div className="rounded-lg border border-slate-600 bg-[#10284f] p-3"><p className="text-slate-400">NotaryOS Pro</p><p className="text-2xl font-bold">${notaryOsMonthlyCost}/mo</p></div>
              <div className="rounded-lg border border-emerald-400/50 bg-emerald-500/10 p-3"><p className="text-slate-300">Estimated Savings</p><p className="text-2xl font-bold text-emerald-300">${monthlySavings}/mo</p></div>
            </div>
          </div>
