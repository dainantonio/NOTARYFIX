  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm" onClick={onClose}>
      <div
        ...
        className={`w-full ${wide ? 'max-w-2xl' : 'max-w-md'} rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-2xl shadow-black/60 overflow-hidden`}
      >
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 px-6 py-4">
          <h3 className="font-black text-slate-900 dark:text-white text-base">{title}</h3>
          <button onClick={onClose} className="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700/70 hover:text-slate-900 dark:hover:text-white transition-all">
            <X className="h-4 w-4" />
          </button>
        </div>
  ...
  return (
    <div className={className}>
      ...
      <input
        ...
        readOnly={readOnly}
        className={`w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-700/40 px-3 py-2.5 text-sm text-slate-900 dark:text-white placeholder:text-slate-500 dark:placeholder:text-slate-400 focus:border-cyan-500/50 focus:outline-none focus:ring-1 focus:ring-cyan-500/30 transition-all ${readOnly ? 'opacity-60 cursor-default' : ''}`}
      />
    </div>
  );
}
  ...
  return (
    <div className={className}>
      ...
      <select
        ...
        className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2.5 text-sm text-slate-900 dark:text-white focus:border-cyan-500/50 focus:outline-none transition-all"
      >
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/[0.05] px-4 py-3 flex items-center gap-3">
          ...
          <div className="min-w-0 ml-auto text-right">
            ...
            <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{liveTrip?.origin || 'Home'}</p>
          </div>
        </div>
    ...
    <Modal open onClose={onClose} title="Split Trip" wide>
      <div className="space-y-5">
        <p className="text-sm text-slate-400">
          Drag to split <strong className="text-slate-900 dark:text-white">{total.toFixed(1)} mi</strong> from{' '}
          <strong className="text-slate-900 dark:text-white">{trip.origin} → {trip.destination}</strong> into two segments.
        </p>
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs font-bold text-slate-900 dark:text-white">{ratio.toFixed(0)}% / {(100 - ratio).toFixed(0)}%</span>
          </div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-3 text-xs text-slate-500">
            <div className="flex items-center justify-between rounded-xl border border-cyan-500/20 bg-cyan-500/5 px-3 py-2">
              ...
              <span className="font-black text-slate-900 dark:text-white">{miles1.toFixed(1)} mi</span>
            </div>
            <div className="flex items-center justify-between rounded-xl border border-slate-500/20 bg-slate-500/5 px-3 py-2">
              ...
              <span className="font-black text-slate-900 dark:text-white">{miles2.toFixed(1)} mi</span>
            </div>
          </div>
        </div>
    ...
    <Modal open onClose={onClose} title="Export Mileage Log" wide>
      <div className="space-y-5">
        ...
        <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              ...
              <p className="text-2xl font-black text-slate-900 dark:text-white">{totalMiles.toFixed(1)}</p>
            </div>
            <div>
              <p className="text-xs text-slate-500 mb-1">IRS Rate (2025)</p>
              <p className="text-2xl font-black text-slate-900 dark:text-white">$0.67</p>
            </div>
            <div>
        ...
        <div>
          ...
          <div className="max-h-40 overflow-y-auto rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 p-4 font-mono text-xs text-slate-400 leading-relaxed">
            {csvContent.split('\n').slice(0, 8).map((row, i) => (
        ...
        <div className="flex justify-end gap-3 flex-wrap">
          <button onClick={copyCSV}
            className="flex items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-700/40 px-4 py-2.5 text-sm font-semibold text-slate-300 hover:bg-slate-200 dark:bg-slate-700/70 transition-all">
...
function TripRow({ trip, onEdit, onDelete, onSplit, onLinkJob, onVerify, onDuplicate }) {
  ...
  return (
    <div className={`group relative rounded-2xl border ${trip.verified ? 'border-slate-200 dark:border-slate-700' : 'border-amber-500/30'} bg-white dark:bg-slate-800 transition-all hover:bg-slate-100 dark:hover:bg-slate-700/60 hover:border-slate-300 dark:border-slate-600`}>
      {!trip.verified && (
      ...
      <div className="flex items-center gap-4 px-5 py-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-sm font-bold text-slate-900 dark:text-white truncate">{trip.destination}</span>
            {trip.linkedJobLabel && (
        ...
        <div className="shrink-0 text-right">
          <p className="text-base font-black text-slate-900 dark:text-white">{parseFloat(trip.miles).toFixed(1)} <span className="text-xs font-medium text-slate-500">mi</span></p>
        ...
        <div className="relative shrink-0">
          <button
            ...
            className="rounded-lg p-1.5 text-slate-600 opacity-0 group-hover:opacity-100 hover:bg-slate-100 dark:hover:bg-slate-700/70 hover:text-slate-900 dark:hover:text-white transition-all"
          >
            <MoreVertical className="h-4 w-4" />
          </button>
          {menuOpen && (
            <div
              className="absolute right-0 top-8 z-20 w-48 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-2xl py-1"
            ...
            >
              {[
                ...
                <button key={item.label} onClick={() => { item.action(); setMenuOpen(false); }}
                  className={`flex w-full items-center gap-2.5 px-3 py-2 text-sm transition-colors hover:bg-slate-100 dark:bg-slate-700/40 ${item.danger ? 'text-rose-400' : 'text-slate-300'}`}>
        ...
        {reminderActive && (
          <div className="flex items-center gap-3 rounded-2xl border border-amber-500/40 bg-amber-500/[0.07] px-5 py-3">
            ...
            <button
              ...
              className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-700/40 px-3 py-1.5 text-xs font-semibold text-slate-300 hover:bg-slate-200 dark:bg-slate-700/70 transition-all"
            >
              Yes, keep going
            </button>
            <button
              onClick={onStop}
              className="rounded-xl bg-rose-500/90 px-3 py-1.5 text-xs font-bold text-slate-900 dark:text-white hover:bg-rose-500 transition-all"
            >
              End trip
            </button>
          </div>
        ...
        <div className="relative overflow-hidden rounded-2xl border border-cyan-500/40 bg-gradient-to-r from-cyan-500/10 via-blue-500/8 to-transparent p-5">
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-base font-black text-slate-900 dark:text-white">{trip.origin} → {trip.destination || 'Destination not set'}</p>
  ...
  return (
    <>
      <div
        ...
        className="group cursor-pointer rounded-2xl border border-dashed border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 p-5 transition-all hover:border-cyan-500/40 hover:bg-cyan-500/5"
      >
        <div className="flex items-center gap-4">
          <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-dashed border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-700/40 group-hover:border-cyan-400/40 group-hover:bg-cyan-400/10 transition-all">
            <Play className="h-6 w-6 text-slate-500 group-hover:text-cyan-400 transition-colors" />
          </div>
          <div>
            <p className="font-bold text-slate-900 dark:text-white group-hover:text-cyan-700 dark:group-hover:text-cyan-100 transition-colors">Start a Trip</p>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Client address, Title company…"
                className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-700/40 px-3 py-2.5 text-sm text-slate-900 dark:text-white placeholder:text-slate-500 dark:placeholder:text-slate-400 focus:border-cyan-500/50 focus:outline-none focus:ring-1 focus:ring-cyan-500/30 transition-all"
              />
            </div>
    ...
    <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-lg font-black text-slate-900 dark:text-white">6-month trend</p>
        </div>
...
export default function Mileage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 p-4 sm:p-6 pb-24 space-y-4 sm:space-y-6">
      ...
      {/* ── VIEW TABS ───────────────────────────────────────────────────────── */}
      <div className="flex gap-1 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-1 w-fit">
        {[{ id: 'log', label: 'Trip Log' }, { id: 'summary', label: 'Summary' }].map(tab => (
          <button key={tab.id} onClick={() => setView(tab.id)}
            className={`rounded-lg px-5 py-2 text-sm font-bold transition-all ${view === tab.id ? 'bg-slate-200 dark:bg-slate-700/70 text-slate-900 dark:text-white' : 'text-slate-500 hover:text-slate-300'}`}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      {view === 'log' ? (
          ...
          <div className="flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[180px] max-w-xs">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                ...
                className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-700/40 py-2.5 pl-9 pr-4 text-sm text-slate-900 dark:text-white placeholder:text-slate-500 dark:placeholder:text-slate-400 focus:border-cyan-500/40 focus:outline-none transition-all" />
            </div>
            <select value={filterPurpose} onChange={e => setFilterPurpose(e.target.value)}
              className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2.5 text-sm text-slate-900 dark:text-white focus:border-cyan-500/40 focus:outline-none transition-all">
        ...
        <div className="space-y-6">
              ...
              {PURPOSES.map(p => {
                ...
                return (
                  <div key={p}>
                    ...
                    <div className="h-2 rounded-full bg-slate-100 dark:bg-slate-700/40 overflow-hidden">
            ...
            <table className="w-full text-sm min-w-[600px]">
              ...
              <tbody>
                {months.map(month => {
                  ...
                  return (
                    <tr key={month} className="border-b border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:bg-slate-800 transition-colors">
                      <td className="px-5 py-3.5 font-semibold text-slate-900 dark:text-white">{monthName(month + '-01')}</td>
                      ...
                      <td className="px-5 py-3.5 text-slate-900 dark:text-white font-semibold">{total.toFixed(1)} mi</td>
              ...
              <tfoot>
                <tr className="border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800">
                  ...
                  <td className="px-5 py-3.5 font-black text-slate-900 dark:text-white">{allYTD.toFixed(1)} mi</td>
          ...
          <div className="rounded-2xl border border-violet-500/20 bg-violet-500/[0.04] p-5 flex gap-4">
            ...
            <div>
              <p className="font-bold text-slate-900 dark:text-white mb-1">IRS Audit-Ready</p>
              <p className="text-sm text-slate-400">
      ...
      {linkModal && (
        <Modal open onClose={() => setLinkModal(null)} title="Attach Trip to Appointment">
          <div className="space-y-4">
            <p className="text-sm text-slate-400">
              Linking <strong className="text-slate-900 dark:text-white">{linkModal.destination}</strong> to an appointment.
            </p>
            <div>
