...
export function createCrudOps(setData) {
  ...
  const createJournalDraftFromAppointment = (appointment) => ({
    ...
    signing_started_at: appointment.signing_started_at || null,
  ...
  const importJurisdictionDataset = (dataset, actor = 'System Import') => setData((p) => {
    ...
    jurisdictions.forEach(([code, rec], i) => {
      ...
      newRules.push({
        ...
        ronPermitted: rec.ron_permitted != null ? Boolean(rec.ron_permitted) : false,
        ronStatute: '',
        seal: '',
        retentionYears: 0,
        ...
        requiresSourceUrl: !rec.officialSourceUrl,
